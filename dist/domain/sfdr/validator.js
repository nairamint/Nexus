    ...
    initializeValidationRules() {
        return [
            ...
            // Sustainability risk integration validation
            {
                ...
                severity: 'ERROR',
                validate: (request) => {
                    const { identificationProcess, assessmentMethodology, integrationInDecisionMaking } = request.sustainabilityRiskIntegration;
            ...
            // Due diligence policies validation
            {
                ...
                severity: 'ERROR',
                validate: (request) => {
                    const { esgIntegration, sustainabilityRisks, adverseImpacts } = request.esgIntegration.dueDiligencePolicies;
            ...
            // Engagement policies validation
            {
                ...
                validate: (request) => {
                    if (['Article8', 'Article9'].includes(request.fundProfile.targetArticleClassification)) {
                        const { shareholderEngagement, votingPolicy } = request.esgIntegration.engagementPolicies;
...
export class RegulatoryComplianceUtils {
     ...
     */
    static validatePAIIndicators(indicators) {
        ...
        return requiredIndicators.every(required => indicators.includes(required));
    }
    /**
    ...
    static validateTaxonomyObjectives(objectives) {
        ...
        const invalidObjectives = objectives.filter(obj => !allObjectives.includes(obj));
     ...
     */
    static formatValidationIssues(issues) {
        if (issues.length === 0) {
        ...
        if (groupedIssues.ERROR.length > 0) {
            ...
            groupedIssues.ERROR.forEach(issue => {
                ...
                if (issue.suggestedAction)
                    report += `  Action: ${issue.suggestedAction}\n`;
                report += '\n';
            });
        }
        // Format warnings
